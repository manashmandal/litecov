...
func formatFooter() string {
	legend := fmt.Sprintf("> **Legend** `ø = not affected`, `new = file not in the base report`, `- = no uncovered lines`\n"+
		"> ✅ ≥ %.0f%%   ⚠️ ≥ %.0f%%   ❌ < %.0f%%\n",
		highCoverageThreshold, mediumCoverageThreshold, mediumCoverageThreshold)
	return "---\n" + legend + "<sub>\U0001F4C8 Generated by [LiteCov](https://github.com/manashmandal/litecov)</sub>\n"
}

// highCoverageThreshold and mediumCoverageThreshold are the cutoffs
// getStatusEmoji applies for its ✅/⚠️/❌ glyphs, and that
// formatFooter's legend restates for the reader. Named and shared between
// the two instead of each hard-coding its own 80/50 so the legend can't
// drift out of sync with what the emoji actually mean (issue #79).
const (
...
func getStatusEmoji(coverage float64) string {
	switch {
	case coverage >= highCoverageThreshold:
		return "\u2705"
	case coverage >= mediumCoverageThreshold:
		return "\u26A0\uFE0F"
	default:
		return "\u274C"
	}
